...
def get_dashboard_metrics(
    ...
    """
    Returns dashboard metrics for the authenticated client.
    ...
    # Note: RLS handles this usually, but explicit filtering is safer/clearer in backend logic.
    ...
    # Execute Queries
    clicks_res = clicks_query.execute()
    ...
    conversion_rate = 0
    if total_clicks > 0:
        conversion_rate = round((total_leads / total_clicks) * 100, 2)

    # 4. Prepare Chart Data (Leads per Day) - Simplified for 'week'/'month'
    # Group by date
    daily_leads = {}
    for l in leads:
