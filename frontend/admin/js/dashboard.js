...
async function loadDashboard() {
    ...
    .then(r => {
        if (r.status === 400) {
            // Master sem impersonação — show friendly redirect
            ["val-clicks","val-leads","val-hot","val-conv","val-abandon"].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.textContent = "—";
            });
            if(window.showToast) window.showToast("Entre no painel de um cliente para ver métricas", "info");
            return null;
        }
